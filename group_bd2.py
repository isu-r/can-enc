...
def main():
    ...
    # --- BD Protocol Round 2 ---
    # Each device computes a T value using a neighbor's public value.
    if device_id == "A":
    ...
    # --- Ephemeral RSA Key Generation ---
    # Prompt for a session identifier (ephemeral nonce)
    session_id = input("Enter a session identifier (ephemeral nonce) for this session: ")
    # Combine the shared key and session_id to create an ephemeral seed.
    seed_material = str(shared_key) + session_id
    seed = int(hashlib.sha256(seed_material.encode()).hexdigest(), 16)
    rng = random.Random(seed)
    
    # Monkey-patch the randomness source so that RSA.generate() is deterministic for this session.
    ...
    print("\nYour Ephemeral RSA public key for session '{}':".format(session_id))
