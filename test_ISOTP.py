...
# In production, use a sufficiently large prime and proper key-derivation and encryption routines.
p = 7919      # a prime number (for demo purposes)
g = 2         # a primitive root modulo p

# A unique delimiter to separate custom data and public key in the message.
DELIMITER = "||PUB||"
...
def main():
    ...
    if mode == 'S':
        ...
        if target not in shared_keys:
            ...
            return
        message = input("Enter the custom message to send: ")
        # Append the sender's public key to the message using the delimiter.
        message_with_key = f"{message}{DELIMITER}{public_key}"
        ciphertext = encrypt_message(message_with_key, shared_keys[target])
        ...
        try:
            decrypted_full = decrypt_message(ciphertext, shared_keys[source])
            # Check if the message contains the appended public key.
            if DELIMITER in decrypted_full:
                message_body, sender_pub = decrypted_full.split(DELIMITER, 1)
                print("\nDecrypted message:")
                print("Message:", message_body)
                print("Sender's public key:", sender_pub)
            else:
                print("\nDecrypted message:")
